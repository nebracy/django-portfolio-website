...
class DoughCalculatorForm(forms.Form):
    choice_set = forms.ChoiceField(label='TF/Weight', choices=TH_FACTOR_DOUGH_WT, initial='th_factor', widget=forms.RadioSelect)
    flour = forms.IntegerField(min_value=100, max_value=100, initial=100, widget=forms.HiddenInput)
    dough_wt_set = forms.DecimalField(label='Dough Weight', min_value=1, max_value=20000, required=False)
    ...
    pizza_size_set = forms.IntegerField(label='Pizza Size (in)', min_value=12, max_value=22, initial=16, required=False)
